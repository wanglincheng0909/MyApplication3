...
public class GitHubDataManager {
     ...
     * 创建或更新用户数据Gist
     */
    public void saveUserData(User user, DataCallback<String> callback) {
        if (!hasCredentials()) {
     ...
     * 同步学习记录到GitHub
     */
    public void syncStudyRecords(String studyRecordsJson, DataCallback<String> callback) {
        if (!hasCredentials()) {
            callback.onError("GitHub凭据未配置");
            return;
        }
     ...
     * 同步错题记录到GitHub
     */
    public void syncErrorRecords(String errorRecordsJson, DataCallback<String> callback) {
        if (!hasCredentials()) {
            callback.onError("GitHub凭据未配置");
            return;
        }
